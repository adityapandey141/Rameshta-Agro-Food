...
const SolutionSection = () => {
  return (
    <section className="max-w-7xl mx-auto mb-[-50px] bg-white py-12 sm:py-16 lg:py-20 px-4 sm:px-6 lg:px-8">
      <div>
        <div className="text-center mb-8 sm:mb-12">
          <h2 className="text-3xl  font-bold mb-4">
            <span className="text-gray-800">What We Do</span>
          </h2>
          <p className="text-gray-500 max-w-3xl text-lg mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-10">
          {BlogPosts.map((post) => (
            <div
              key={post.id}
              className="bg-white shadow-md rounded-md overflow-hidden relative hover:shadow-lg transition duration-300"
            >
              ...
              <div className="relative h-56 sm:h-60 lg:h-72 w-full flex justify-between">
                <div className="w-full flex justify-between transition-transform duration-300 hover:scale-105">
                  <Image
                    src={post.image}
                    alt={post.title}
                    fill
              ...
              <div className="px-4 sm:px-6 py-6">
                ...
                <Link href={`/blog/${post.slug}`}>
                  <h3 className="text-xl sm:text-2xl font-bold mb-4 text-gray-800 hover:text-yellow-500 transition duration-300 transform hover:scale-105 cursor-pointer">
                    {post.title}
                  </h3>
                </Link>

                <p className="text-gray-600 text-lg mb-6 transition-transform duration-300 hover:scale-105 cursor-pointer">
                  {post.excerpt}
                </p>

                <div className="flex">
                  <Link href="/contact-us">
                    <span className="border border-yellow-400 text-gray-700 hover:bg-yellow-400 hover:text-white px-6 py-2 rounded-full text-sm sm:text-base font-medium transition duration-300 cursor-pointer">
                      READ MORE
                    </span>
                  </Link>
                </div>
